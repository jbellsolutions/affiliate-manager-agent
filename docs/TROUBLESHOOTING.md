...
template tracks the **live** ai-guy-go-to-market (Hermes-native gateway), which is why it
...
| Gateway **crash-loops on boot** | A channel is `enabled: true` in config but its token is missing/blank. (Zero enabled channels is *not* the cause — a fully headless agent, cron/dashboard only, boots and stays healthy.) | Only enable channels you have tokens for. `new-agent.sh` does this automatically from `agent.env`; if you hand-edited config, match `platforms.*.enabled` to the tokens present, or set both to `false` and run headless. |
