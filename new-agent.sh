#!/usr/bin/env bash
# new-agent.sh — stamp out a new Affiliate Manager agent from one config file.
#
...
# Role files — seed once so reruns never overwrite the operator's live context.
for role_file in SOUL.md AGENTS.md; do
  if [ -f "$BASE_DIR/hermes/data/$role_file" ]; then
    echo "  · keeping existing $BASE_DIR/hermes/data/$role_file"
  else
    cp "$TEMPLATE_DIR/files/$role_file" "$BASE_DIR/hermes/data/$role_file"
